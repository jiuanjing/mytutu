...
<%
    ...
    date = date.length() == 0 ? DateTool.getPreYear() : date;
    String kpiId = new String(request.getParameter("kpi").getBytes("ISO-8859-1"), "utf-8");
    if (dbOperation.dbOpen()) {
        try {
            ...
            String sql = " select t1.brief_name, t.actual_value, t.comp_score " +
                    "  from echarts.dm_op_yr_evaluate t, echarts.dim_op_company t1, echarts.dim_op_kpi t2 " +
                    " where t.date_id = " + date +
                    "   and t2.kpi_id = '" + kpiId + "' " +
                    "   and t.company_id = t1.company_id and t1.flag_sewage = 1" +
                    "   and t.kpi_id = t2.kpi_id and t2.kpi_type = 2 " +
                    " order by t.comp_score ";
            //公司id为-1的实际值代表该kpi的标杆值
            String sql1 = " select t.comp_score " +
                    "  from echarts.dm_op_yr_evaluate t, echarts.dim_op_kpi t2 " +
                    " where t.date_id = " + date +
                    "   and t2.kpi_id = '" + kpiId + "' " +
                    "   and t.company_id = -1 " +
                    "   and t.kpi_id = t2.kpi_id and t2.kpi_type = 2 " +
