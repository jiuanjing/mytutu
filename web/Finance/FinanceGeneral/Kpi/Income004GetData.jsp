...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            sqlstr = "select t.brief_name,round(t.operating_revenue/10000,2),round(t.operating_revenue_budget/10000,2)" +
                    ",round((t.OPERATING_REVENUE/t.OPERATING_REVENUE_BUDGET)*100,2) from (select * from " +
                    tableName + " a,echarts.dim_company_fn b " +
                    "where a.company_id = b.company_id  ";


            sqlstr = sqlstr + where + " and B.FLAG_DISPLAY = 1 and b.status=1 and b.company_level=2 order by "
                    + sort + " " + order + ") t where T.OPERATING_REVENUE >0 AND rownum<=10";
