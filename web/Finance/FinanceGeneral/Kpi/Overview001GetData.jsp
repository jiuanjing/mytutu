...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            String sqlstr = "";
            //echarts.dm_operating_growth
            String tableNames = "";
            String where = "";

            String date = request.getParameter("date").trim();
            String types = request.getParameter("sort").trim();

            if (date != null && !(date.equals(""))) {
                if ("0".equals(types)) {
                    tableNames = "echarts.dm_operating_growth";
                ...
                if ("1".equals(types)) {
                    tableNames = "echarts.dm_operating_growth_q";
                    int begin_date_int = Integer.valueOf((Integer.valueOf(date.substring(0, 4)) - 1)
                            + DateTool.getSeason(date.substring(5, 7)));
                    where += " and date_id >" + begin_date_int;
                    int end_date_int = Integer.valueOf(date.substring(0, 4) + DateTool.getSeason(date.substring(5, 7)));
                    where += " and date_id <=" + end_date_int;
                }
                if ("2".equals(types)) {
                    tableNames = "echarts.dm_operating_growth_y";
                }
            }
            sqlstr = "select date_id,round(total_profit_ty/10000,2)," +
                    "tpir from " + tableNames + " where company_id=1";
