...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            sqlstr = "select c.bu_name,round(sum(a.TOTAL_ASSET)/10000,2) from " + tableName + " " +
                    "a ,echarts.dim_company_fn b,echarts.dim_bu c "
                    + "where a.company_id = b.company_id and B.FLAG_DISPLAY = 1 and b.bu_id = c.bu_id and a.company_id > 2";
            sqlstr = sqlstr + where + " and b.status=1 and b.company_level=2 and " +
                    "c.status=1 and c.analysis_flag=1 group by c.bu_name order by sum(a.TOTAL_ASSET) desc";
