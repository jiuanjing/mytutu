...
<%
    ...
    if (db.dbOpen()) {
        try {
            ...
            //归属母公司净利润
            String sql1 = "select round(PROFIT_PARENT/10000,2) from " + tableName + " t" + where;
            //管理费用
            String sql2 = "select round(MANAGEMENT_EXPENSE/10000,2) from " + tableName + " t" + where;
            //单位变动成本
            String sql3 = "select round(VAR_COST_PER_UNIT,2) from " + tableName + " t" + where;
            //单位总成本
            String sql4 = "select round(COST_PER_UNIT,2) from " + tableName + " t" + where;
            //营业收入
            String sql5 = "select round(OPERATING_REVENUE/10000,2) from " + tableName + " t" + where;
            sqlList.add(sql1);
            sqlList.add(sql2);
            sqlList.add(sql3);
            sqlList.add(sql4);
            ...
            for (String aSqlList : sqlList) {
                ResultSet rs = null;
                rs = db.executeQuery(aSqlList);//通过数据库访问程序返回一个可滚动的记录集
                if (rs == null) {
