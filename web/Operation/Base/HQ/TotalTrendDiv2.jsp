<%@ page import="com.bws.dbOperation.DBOperation" %>
<%@ page import="java.sql.PreparedStatement" %>
<%@ page import="java.sql.ResultSet" %>
<%@ page import="java.sql.SQLException" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.HashMap" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.ArrayList" %>
<%@ page import="com.google.gson.Gson" %>
<%@ page import="com.bws.util.CollectionUtil" %><%--
  Created by wang at 2016/10/15 15:09
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%
    String year = request.getParameter("year");

    DBOperation db = new DBOperation(true);
    if (db.dbOpen()) {
        try {
            String sql = "SELECT NVL(V1.SCALE, 0), " +
                    "    NVL(V1.ACTUAL, 0), " +
                    "    V2.DEPT_ID, " +
                    "    V2.DEPT_NAME, " +
                    "    V2.TYPE_ID " +
                    "  FROM (SELECT SUM(T.SCALE) SCALE, " +
                    "      T2.DEPT_NAME, " +
                    "      SUM(T.SCALE_ACTUAL) ACTUAL, " +
                    "      T.BU_ID BU_ID, " +
                    "      T2.DEPT_ID DEPT_ID " +
                    "    FROM ECHARTS.DM_OP_YR_BASE_SCALE_DEPT T, ECHARTS.DIM_DEPT_OP T2 " +
                    "   WHERE T.DATE_ID = ? " +
                    "     AND T.DEPT_ID = T2.DEPT_ID " +
                    "   GROUP BY T.BU_ID, T2.DEPT_NAME, T2.DEPT_ID) V1, " +
                    "    (SELECT T1.DEPT_ID, T3.TYPE_ID, T1.DEPT_NAME " +
                    "    FROM ECHARTS.DIM_DEPT_OP T1, ECHARTS.DIM_OP_COMPANY_TYPE T3) V2 " +
                    " WHERE V1.DEPT_ID(+) = V2.DEPT_ID " +
                    "   AND V1.BU_ID(+) = V2.TYPE_ID " +
                    " ORDER BY V2.TYPE_ID, V2.DEPT_ID";
            PreparedStatement preparedStatement = db.getPrepareStmt(sql);
            preparedStatement.setString(1, year);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet != null) {
                Map<String, List<String>> map = new HashMap<String, List<String>>();
                List<String> list1 = new ArrayList<String>();
                List<String> list2 = new ArrayList<String>();
                List<String> list3 = new ArrayList<String>();
                while (resultSet.next()) {
                    list1.add(resultSet.getString(4));
                    list2.add(resultSet.getString(1));
                    list3.add(resultSet.getString(2));
                }
                map.put("deptName", list1);
                map.put("scale", list2);
                map.put("actualScale", list3);
                Gson gson = new Gson();
                String returnData = gson.toJson(map);
                out.write(returnData);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            db.dbClose();
        }
    }
%>
