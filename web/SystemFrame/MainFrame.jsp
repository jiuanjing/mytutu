...
**********************************************************************************************************************/
%>
<%@ page language="java"  contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" import="java.sql.*,com.bws.util.*,com.bws.dbOperation.*,com.bws.tools.*"%>
	...
	<div class="north" data-options="region:'north',border:false">
		...
		<div class="north_nav">
			...
			<% 
					...
					{
					%>
						<li id="nav_li<%=rs_level1.getString(1) %>" class="nav_li<%=rs_level1.getString(1) %>" onclick="javascript:showChildMenu(<%=rs_level1.getString(1) %>)">&nbsp;&nbsp;&nbsp;&nbsp;<%=rs_level1.getString(3) %>
											...
											while(rs_level4.next())
											{
												...
												if(i5>=1)
												{
													...
													while(rs_level5.next())
													{
														...
														/***Level6六级菜单开始****/
												        String sql_count6 = "select count(*) from bim.user_info a,bim.role_module c,bim.module d";
														sql_count6 = sql_count6 + " where a.user_id=" + UserID + " and a.role_id=c.role_id and c.module_id=d.module_id and d.status=1 and c.right_type=1";
														sql_count6 = sql_count6 + " and d.module_level=6 and substr(module_code,1,10)='" + rs_level5.getString(2) + "'";

														String sql_level6 = "select d.module_id,module_code,module_name,URL,access_method from bim.user_info a,bim.role_module c,bim.module d";
														sql_level6 = sql_level6 + " where a.user_id=" + UserID + " and a.role_id=c.role_id and c.module_id=d.module_id and d.status=1 and c.right_type=1";
														sql_level6 = sql_level6 + " and d.module_level=6 and substr(module_code,1,10)='" + rs_level5.getString(2) + "'";
														sql_level6 = sql_level6 + " order by d.order_no";
											
														ResultSet rs_count6 = null;
														ResultSet rs_level6 = null;
											
														rs_count6 = db.executeQuery(sql_count6);//
														rs_level6 = db.executeQuery(sql_level6);//通过数据库访问程序返回一个可滚动的记录集
														...
														if(rs_count6.next())
														{
															i6 = rs_count6.getInt(1);
														}
														if(i6>=1)
														{
															str_navi_tree_html = str_navi_tree_html + "<ul  class=\"easyui-tree\">";
															while(rs_level6.next())
															{
																if((rs_level6.getString(4) != null) && (rs_level6.getString(4).trim() != ""))
																{
																	if(rs_level6.getString(5).equals("OLAP"))
																	{
																		str_navi_tree_html = str_navi_tree_html + "<li><span><a onclick=\"window.frames['contentFrame'].location.href='" + rs_level6.getString(4) + "&CAMUsername=" + UserAccount + "&CAMPassword=" + UserPassword + "'\">" + rs_level6.getString(3) + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a></span>";
																	}
																	else
																	{
																		str_navi_tree_html = str_navi_tree_html + "<li><span><a onclick=\"window.frames['contentFrame'].location.href='" + rs_level6.getString(4) + "'\">" + rs_level6.getString(3) + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a></span>";
																	}
																}
																else
																{
																	str_navi_tree_html = str_navi_tree_html + "<li><span>" + rs_level6.getString(3) + "</span>";
																}														
																/***如果要加7级菜单，可以从这里开始****/
																/***如果要加7级菜单，这里写6级菜单的代码，拷贝6级菜单，适当修改即可****/
...
<!-- 		<div id="mm" style="width:130px;">
		...
		<div class="north_info">
			<div style="margin-left: 85px;margin-top: 40px;color: #ffffff;font-size: 16px;font-weight:bold;" ><a style="color:#ffffff;" href="javascript:void(0)" id="mb"><%=UserName %></a></div>
		</div>
	</div>
	<div id="childMenu" name="childMenu" title="导航" data-options="region:'west',split:true,border:false" style="width:200px;">
		<%=str_navi_tree_html %>
	</div>
	<div id="main" name="main" class="center_main" data-options="region:'center',split:false,border:false">
		<iframe id="contentFrame" name="contentFrame" height="99%" width="100%" frameborder="0" scrolling="auto" marginheight="0" marginwidth="0">
		</iframe>
	</div>
</body>
</html>
